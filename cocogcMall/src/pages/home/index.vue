  ...
  data() {
    return {
      fastList: [
        ...
        { id: 10, name: "积分公益", img: 'jfgy', active: false, path: 'javascript:;' },
        ...
        .then((data) => {
          if (data.resultCode == 0) {
            ...
            this.newsList.forEach(res => {
              if(res.status == 0){
                 html += `<a href=${res.url.length==0||res.url==null?'javascript:void(0)':res.url} class="link-url" style="color:#fff;margin-right: 50px;">
                       ${res.noticeTitle}
                    </a>`
              }
            });
